...
The handler gives its model Configure profile tools plus the connector/action capabilities this sample supports, so the agent can read, search, remember, and use approved connected capabilities before replying. It still reads approved orientation sections and formats the returned packet for first-turn UX instead of putting the raw profile response in the prompt. Use `configure_profile_read` and `configure_profile_search` for overview, concrete memories, imported-source questions, and details that need exact source attribution. After a read-backed reply, it calls `ctx.profile.commit()` with bounded user/assistant turn evidence. The formatted context labels linked approved profile context versus developer-scoped unlinked context when visible context exists.
