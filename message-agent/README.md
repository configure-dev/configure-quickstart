...
The handler gives its model Configure tools, so the agent can search and remember user context before replying. It reads a small `identity`/`preferences`/`summary` overview and formats a bounded Configure context packet instead of putting the raw profile response in the prompt. The canonical SDK shape is `profile.format({ guidelines: false, maxChars: 6_000 })`; this sample also clamps locally so it stays safe while older published SDK types are still in circulation. Use `configure_profile_search` for concrete memories, imported-source questions, and details that need completeness/source attribution. The formatted context labels linked approved profile context versus developer-scoped unlinked context when visible context exists.
